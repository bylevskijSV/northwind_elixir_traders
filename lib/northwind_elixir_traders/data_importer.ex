defmodule NorthwindElixirTraders.DataImporter do
  ...
  def start,
    do: if(is_nil(Process.whereis(@name)), do: Repo.start_link(name: @name, database: @database))
  ...
  def import_all_modeled(), do: get_tables_to_import() |> Enum.map(&insert_all_from/1)
